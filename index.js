const IMGBB_API_KEY = "1bc284892c5e7152155b0d57e9bfc02d";
...
app.get("/admin/edit/:id", (req, res) => {
  ...
  db.query("SELECT * FROM products WHERE id = ?", [productId], (err1, productResults) => {
    if (err1 || productResults.length === 0) {
    ...
    db.query("SELECT image_url FROM product_images WHERE product_id = ?", [productId], (err2, imageResults) => {
      const productImages = imageResults || [];
...
app.post("/admin/edit/:id", (req, res) => {
  upload.array("images", 5)(req, res, function (err) {
    ...
    const productId = req.params.id;
    const { name, price, description, featured, target_group, category, sale_price } = req.body;

    const sql = `
      UPDATE products
      SET name = ?, price = ?, description = ?, featured = ?, target_group = ?, category = ?, sale_price = ?
      WHERE id = ?
    `;
    ...
    db.query(sql, values, (err) => {
      ...
      if (req.files && req.files.length > 0) {
        const imageFilenames = req.files.map(file => "/uploads/" + file.filename);
        const imageSql = `INSERT INTO product_images (product_id, image_url) VALUES ?`;
        const imageValues = imageFilenames.map(url => [productId, url]);

        db.query(imageSql, [imageValues], (err2) => {
          if (err2) {
            console.error("❌ Error saving new images:", err2);
            return res.status(500).send("❌ Product updated but failed to save new images");
          }
          return res.redirect("/admin/dashboard?action=view&success=updated");
        });
      } else {
        return res.redirect("/admin/dashboard?action=view&success=updated");
      }
...
app.get("/admin/dashboard", (req, res) => {
  ...
  if (action === "view") {
    ...
    db.query(ordersSql, (err1, orders) => {
...
const openai = new OpenAI({
  apiKey: "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" // replace with your actual key
});
